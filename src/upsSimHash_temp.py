...
class UnweightedPrioritySamplingSimHash():
    ...
    def sketch(self, vector: np.ndarray) -> UPSSHSketch:
        ...
        abs_rank = [abs(rank[i]) for i in range(len(vector))]
        tau = float('inf')  
        abs_rank = sorted(abs_rank[i] for i in range(len(vector)))
        ...
        for i in range(len(rank)):
            if abs(rank[i]) < tau:
                sk_indices.append(i)
                sk_values.append(np.sign(rank[i])*vector[i])
        
        return UPSSHSketch(sk_indices, sk_values, np.linalg.norm(vector), tau)
