...
class QWMH_L1():
    ...
    def sketch(self, vector: np.ndarray) -> QWMHSketch:   
        ...
        sk_hashes = np.zeros(self.sketch_size)
        
        # generate sk_hashes to be either 1 or -1 based on random seed generated by all_min_indices for each index
        for ind in range(self.sketch_size):
            random.seed(all_min_indices[ind] * (1000000) + self.seed * self.sketch_size)
            sk_hashes[ind] = np.random.choice([-1, 1])
