...
def plot_average_error(sh, jl, ps):
    ...
    plt.title('Comparison of Average Relative Error (abs(difference)/(vecA_l2*vecB_l2)')
    ...
    plt.savefig('./results/storage_average_new_error_.png')


sh = pd.read_csv('./results/storage_new_error_SimHash.csv')
jl = pd.read_csv('./results/storage_new_error_JL.csv')
ps = pd.read_csv('./results/storage_new_error_PrioritySampling.csv')
